...
def train(args, model, device, train_data, dev_data, test_data, processor):
    ...
    if args.optimizer_name == 'adafactor':
        ...
        if args.model == 'MSD_model':
            ...
            optimizer = AdamW([
                    ...
                    ], lr=args.learning_rate, weight_decay=args.weight_decay)  # type: ignore
    ...
    for i_epoch in trange(0, int(args.num_train_epochs), desc="Epoch", disable=False):
        ...
        print('train_loss: '+str(sum_loss/sum_step)+"\n")
        dev_acc, dev_f1 ,dev_precision,dev_recall = evaluate_acc_f1(args, model, device, dev_data, processor, mode='dev')
        print('dev_acc: '+str(dev_acc)+' dev_f1: '+str(dev_f1)+' dev_precision: '+str(dev_precision)+' dev_recall: '+str(dev_recall)+"\n")
        ...
        if dev_acc > max_acc:
            ...
            print('test_acc: '+str(test_acc)+' macro_test_f1: '+str(test_f1)+
                     ' macro_test_precision: '+str(test_precision)+' macro_test_recall: '+str(test_recall)+"\n")
            print(' micro_test_f1: '+str(test_f1_)+' micro_test_precision: '+str(test_precision_)+
                  ' micro_test_recall: '+str(test_recall_)+"\n")
...
def evaluate_acc_f1(args, model, device, data, processor, macro=False,pre = None, mode='test'):
        ...
        if mode == 'test':
            print('test_loss: '+str(sum_loss/sum_step)+'\n')
        else:
            print('dev_loss: '+str(sum_loss/sum_step)+'\n')
        if pre != None:
